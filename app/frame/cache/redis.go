...
// 参考来源 https://github.com/aiscrm/redisgo
package cache
...
type Options struct {
	...
	Addr        string                                 // redis服务的地址，默认为 127.0.0.1
	Port        int                                    // redis服务的端口，默认为 6379
	...
	case Options:
		...
		pool := &redis.Pool{
			...
			Dial: func() (redis.Conn, error) {
				conn, err := redis.Dial(opts.Network, fmt.Sprintf("%s:%d", opts.Addr, opts.Port))
				if err != nil {
					return nil, err
				}
				if opts.Password != "" {
